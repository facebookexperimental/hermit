...
static void verify_thread_futexes(void) {
  ...
  check_pthread(
      pthread_barrier_init(&state.barrier, NULL, THREADS + 1),
      "pthread_barrier_init");
  check_pthread(pthread_mutex_init(&state.mutex, NULL), "pthread_mutex_init");

  for (int i = 0; i < THREADS; ++i) {
    check_pthread(
        pthread_create(&threads[i], NULL, thread_worker, &state),
        "pthread_create");
  }
  ...
  if (state.counter != expected) {
    fprintf(
        stderr,
        "thread counter: expected %ld, got %ld\n",
        expected,
        state.counter);
    exit(1);
  }
  ...
  check_pthread(
      pthread_barrier_destroy(&state.barrier), "pthread_barrier_destroy");
}

static void verify_process_shared_futexes(void) {
  struct process_state* state = mmap(
      NULL,
      sizeof(*state),
  ...
  check_pthread(
      pthread_mutexattr_init(&mutex_attributes), "pthread_mutexattr_init");
  check_pthread(
      pthread_mutexattr_setpshared(&mutex_attributes, PTHREAD_PROCESS_SHARED),
      "pthread_mutexattr_setpshared");
  check_pthread(
      pthread_condattr_init(&condition_attributes), "pthread_condattr_init");
  check_pthread(
      pthread_condattr_setpshared(
          &condition_attributes, PTHREAD_PROCESS_SHARED),
      "pthread_condattr_setpshared");
  check_pthread(
      pthread_mutex_init(&state->mutex, &mutex_attributes),
      "pthread_mutex_init");
  check_pthread(
      pthread_cond_init(&state->condition, &condition_attributes),
      "pthread_cond_init");
  ...
  if (child == 0) {
    ...
    state->child_ready = 1;
    check_pthread(
        pthread_cond_broadcast(&state->condition), "child condition broadcast");
  ...
  check_pthread(
      pthread_cond_broadcast(&state->condition), "parent condition broadcast");
  ...
  check_pthread(
      pthread_cond_destroy(&state->condition), "pthread_cond_destroy");
  check_pthread(pthread_mutex_destroy(&state->mutex), "pthread_mutex_destroy");
  check_pthread(
