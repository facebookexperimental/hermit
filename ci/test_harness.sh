...
function audit_ci_correspondence {
    ...
    [[ $(grep -Fc '0fd04fee8837db7d31aa62faac86d009f015a9f4' "$budget_wrapper") == 1 ]] ||
        die "DBT wrapper must name exactly one calibrated Reverie pin"
    [[ $(grep -Fc '0fd04fee8837db7d31aa62faac86d009f015a9f4' "$budget_config") == 2 ]] ||
    ...
    (
        ...
        budget_tuple=$(
            PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
                REVERIE_DBT_BUDGET_BOUND_PIN=0fd04fee8837db7d31aa62faac86d009f015a9f4 \
        ...
        [[ $hosted_wrapper_log == *'pin:0fd04fee8837db7d31aa62faac86d009f015a9f4,source:inherited-launch-cargo-build-jobs,raw-build-jobs:8,effective-cpus-source:child-nproc,effective-cpus:4,reverie-max-jobs:16,effective-native-jobs:4,effective-job-seconds:1050,max-elapsed-seconds:263'* ]] ||
        ...
        [[ $boxed_wrapper_log == *'pin:0fd04fee8837db7d31aa62faac86d009f015a9f4,source:runner-child-cargo-build-jobs,raw-build-jobs:32,effective-cpus-source:child-nproc,effective-cpus:64,reverie-max-jobs:16,effective-native-jobs:16,effective-job-seconds:1050,max-elapsed-seconds:66'* ]] ||
...
EOF
        ...
        if PATH="$scratch/nproc-zero:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBT_BUDGET_BOUND_PIN=0fd04fee8837db7d31aa62faac86d009f015a9f4 CARGO_BUILD_JOBS=8 \
            bash -c 'source "$1" reverie-dbt-budget-child' _ "$budget_config" 2>/dev/null; then
        ...
        if PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBT_BUDGET_BOUND_PIN=0fd04fee8837db7d31aa62faac86d009f015a9f4 CARGO_BUILD_JOBS=0 \
            bash -c 'source "$1" reverie-dbt-budget-child' _ "$budget_config" 2>/dev/null; then
