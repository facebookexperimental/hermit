...
function audit_ci_correspondence {
    ...
    [[ $(grep -Fc '9470712afa9b421c72850ab7955fb335692e43a0' "$budget_wrapper") == 1 ]] ||
        die "DBI wrapper must name exactly one calibrated Reverie pin"
    [[ $(grep -Fc '9470712afa9b421c72850ab7955fb335692e43a0' "$budget_config") == 2 ]] ||
        die "DBI derivation must independently require and diagnose the calibrated Reverie pin"
    # shellcheck disable=SC2016
    local budget_record='reverie-dbi-budget={pin:$REVERIE_DBI_BUDGET_BOUND_PIN,source:$REVERIE_DBI_BUILD_JOBS_SOURCE,raw-build-jobs:$REVERIE_DBI_RAW_BUILD_JOBS,effective-cpus-source:$REVERIE_DBI_EFFECTIVE_CPUS_SOURCE,effective-cpus:$REVERIE_DBI_EFFECTIVE_CPUS,reverie-max-jobs:$REVERIE_DBI_MAX_PARALLEL_JOBS,effective-native-jobs:$REVERIE_DBI_EFFECTIVE_BUILD_JOBS,effective-job-seconds:$REVERIE_DBI_MAX_BUILD_EFFECTIVE_JOB_SECONDS,max-elapsed-seconds:$REVERIE_DBI_MAX_BUILD_SECONDS,basis:github-portable-cold-miss-n3-affinity4,carried-to-pin-on-dynamorio-recipe-key:19123c88d87a4cd9}'
    ...
    (
        ...
        budget_tuple=$(
            PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
                REVERIE_DBI_BUDGET_BOUND_PIN=9470712afa9b421c72850ab7955fb335692e43a0 \
        ...
        [[ $hosted_wrapper_log == *'pin:9470712afa9b421c72850ab7955fb335692e43a0,source:inherited-launch-cargo-build-jobs,raw-build-jobs:8,effective-cpus-source:child-nproc,effective-cpus:4,reverie-max-jobs:16,effective-native-jobs:4,effective-job-seconds:1050,max-elapsed-seconds:263'* ]] ||
        ...
        [[ $boxed_wrapper_log == *'pin:9470712afa9b421c72850ab7955fb335692e43a0,source:runner-child-cargo-build-jobs,raw-build-jobs:32,effective-cpus-source:child-nproc,effective-cpus:64,reverie-max-jobs:16,effective-native-jobs:16,effective-job-seconds:1050,max-elapsed-seconds:66'* ]] ||
        ...
        if PATH="$scratch/nproc-zero:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBI_BUDGET_BOUND_PIN=9470712afa9b421c72850ab7955fb335692e43a0 CARGO_BUILD_JOBS=8 \
            bash -c 'source "$1" reverie-dbi-budget-child' _ "$budget_config" 2>/dev/null; then
        ...
        if PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBI_BUDGET_BOUND_PIN=9470712afa9b421c72850ab7955fb335692e43a0 CARGO_BUILD_JOBS=0 \
            bash -c 'source "$1" reverie-dbi-budget-child' _ "$budget_config" 2>/dev/null; then
