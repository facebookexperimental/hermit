...
function audit_ci_correspondence {
    ...
    [[ $(grep -Fc 'ab44bbf7351e2329a0de5270d22a02a0c8142f4a' "$budget_wrapper") == 1 ]] ||
        die "DBT wrapper must name exactly one calibrated Reverie pin"
    [[ $(grep -Fc 'ab44bbf7351e2329a0de5270d22a02a0c8142f4a' "$budget_config") == 2 ]] ||
    ...
    (
        ...
        budget_tuple=$(
            PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
                REVERIE_DBT_BUDGET_BOUND_PIN=ab44bbf7351e2329a0de5270d22a02a0c8142f4a \
        ...
        [[ $hosted_wrapper_log == *'pin:ab44bbf7351e2329a0de5270d22a02a0c8142f4a,source:inherited-launch-cargo-build-jobs,raw-build-jobs:8,effective-cpus-source:child-nproc,effective-cpus:4,reverie-max-jobs:16,effective-native-jobs:4,effective-job-seconds:1050,max-elapsed-seconds:263'* ]] ||
        ...
        [[ $boxed_wrapper_log == *'pin:ab44bbf7351e2329a0de5270d22a02a0c8142f4a,source:runner-child-cargo-build-jobs,raw-build-jobs:32,effective-cpus-source:child-nproc,effective-cpus:64,reverie-max-jobs:16,effective-native-jobs:16,effective-job-seconds:1050,max-elapsed-seconds:66'* ]] ||
        ...
        if PATH="$scratch/nproc-zero:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBT_BUDGET_BOUND_PIN=ab44bbf7351e2329a0de5270d22a02a0c8142f4a CARGO_BUILD_JOBS=8 \
            bash -c 'source "$1" reverie-dbt-budget-child' _ "$budget_config" 2>/dev/null; then
        ...
        if PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBT_BUDGET_BOUND_PIN=ab44bbf7351e2329a0de5270d22a02a0c8142f4a CARGO_BUILD_JOBS=0 \
            bash -c 'source "$1" reverie-dbt-budget-child' _ "$budget_config" 2>/dev/null; then
