...
function audit_ci_correspondence {
    ...
    [[ $(grep -Fc '6a6b4ec3e1806aacec0299cc46359a8f57e968fe' "$budget_wrapper") == 1 ]] ||
        die "DBI wrapper must name exactly one calibrated Reverie pin"
    [[ $(grep -Fc '6a6b4ec3e1806aacec0299cc46359a8f57e968fe' "$budget_config") == 2 ]] ||
    ...
    (
        ...
        budget_tuple=$(
            PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
                REVERIE_DBI_BUDGET_BOUND_PIN=6a6b4ec3e1806aacec0299cc46359a8f57e968fe \
        ...
        [[ $hosted_wrapper_log == *'pin:6a6b4ec3e1806aacec0299cc46359a8f57e968fe,source:inherited-launch-cargo-build-jobs,raw-build-jobs:8,effective-cpus-source:child-nproc,effective-cpus:4,reverie-max-jobs:16,effective-native-jobs:4,effective-job-seconds:1050,max-elapsed-seconds:263'* ]] ||
        ...
        [[ $boxed_wrapper_log == *'pin:6a6b4ec3e1806aacec0299cc46359a8f57e968fe,source:runner-child-cargo-build-jobs,raw-build-jobs:32,effective-cpus-source:child-nproc,effective-cpus:64,reverie-max-jobs:16,effective-native-jobs:16,effective-job-seconds:1050,max-elapsed-seconds:66'* ]] ||
        ...
        if PATH="$scratch/nproc-zero:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBI_BUDGET_BOUND_PIN=6a6b4ec3e1806aacec0299cc46359a8f57e968fe CARGO_BUILD_JOBS=8 \
            bash -c 'source "$1" reverie-dbi-budget-child' _ "$budget_config" 2>/dev/null; then
        ...
        if PATH="$scratch/nproc-4:$PATH" "${clean_budget_env[@]}" \
            REVERIE_DBI_BUDGET_BOUND_PIN=6a6b4ec3e1806aacec0299cc46359a8f57e968fe CARGO_BUILD_JOBS=0 \
            bash -c 'source "$1" reverie-dbi-budget-child' _ "$budget_config" 2>/dev/null; then
