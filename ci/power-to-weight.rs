...
//!
//! Each portable-DAG node has a COST (weight) and a VALUE (power). This tool
//! joins the two so low-value / high-cost nodes can be identified and moved off
//! the per-commit critical path (e.g. onto a nightly lane).
//!
//!   * WEIGHT  = `hint.est_duration_s` from `ci/dag/portable.json`. These are
//!               HAND-ESTIMATED (see ci/dag/README), not measured — treat them
//!               as ordinal, not absolute seconds. Replace with observed
//!               durations from the validate-run-ledger when available.
...
//!   * WEIGHT is an estimate; the ranking is only as good as those estimates.
//!
//! Usage:
...
fn load_nodes(dag_path: &Path) -> BTreeMap<String, Node> {
    ...
    for s in v["steps"].as_array().unwrap_or(&vec![]) {
        ...
        nodes.insert(
            ...
            Node {
                tag,
                est_duration_s: hint["est_duration_s"].as_u64().unwrap_or(0),
...
Rank portable-DAG nodes by power-to-weight: selection frequency over recent
history (POWER) against hand-estimated duration (WEIGHT). Flags expensive,
rarely-selected nodes as nightly candidates.
...
WEIGHT is hand-estimated (ci/dag/README); POWER is measured on PAST commits and
predicts future value only if the change mix is stable. Low power-to-weight means
'consider moving to nightly', never 'safe to delete'.
"
        );
        return;
    }
    ...
    // Normalize weight to the max duration so power-to-weight is unit-free.
    let max_dur = nodes.values().map(|n| n.est_duration_s).max().unwrap_or(1).max(1) as f64;
    ...
    let p2w = |n: &Node| {
        ...
        let w = (n.est_duration_s as f64 / max_dur).max(1e-9);
        rate / w
    };
    ranked.sort_by(|a, b| {
        p2w(a).partial_cmp(&p2w(b)).unwrap().then(b.est_duration_s.cmp(&a.est_duration_s))
    });

    // A "nightly candidate" is expensive AND rarely selected.
    let is_nightly_candidate = |n: &Node| {
        let rate = n.times_selected as f64 / total;
        n.est_duration_s >= 120 && rate < 0.34
    };

    match format.as_str() {
        "csv" => {
            println!("node,est_duration_s,classification,times_selected,sample_size,selection_rate,power_to_weight,nightly_candidate");
            for n in &ranked {
                let rate = n.times_selected as f64 / total;
                println!(
                    "{},{},{},{},{},{:.3},{:.3},{}",
        ...
        "human" => {
            ...
            println!("  WEIGHT = est_duration_s (HAND-ESTIMATED, ordinal); POWER = selection rate over sample.\n");
            println!(
                "{:<38} {:>6} {:>14} {:>9} {:>7}  {}",
                "node", "dur_s", "classification", "sel_rate", "p2w", "flag"
            );
            for n in &ranked {
                let rate = n.times_selected as f64 / total;
            ...
            println!(
                "\n{} node(s) flagged NIGHTLY-CANDIDATE (dur ≥ 120s AND selected < 34% of the sample).",
                candidates.len()
            );
            println!(
                "These are ranked, not condemned: moving one to nightly trades per-commit latency for\n\
                 ...
                 and prefer real durations from the validate-run-ledger over these estimates."
            );
        }
