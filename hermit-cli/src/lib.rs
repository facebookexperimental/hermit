...
#[cfg(fbcode_build)]
pub fn reserve_kvm_stdin(_stdin: Option<fs::File>) -> std::io::Result<()> {
    Ok(())
}

#[cfg(fbcode_build)]
fn ensure_kvm_stdin_reserved() -> std::io::Result<()> {
    Ok(())
}
...
#[cfg(fbcode_build)]
fn dbi_runtime_unavailable_reason() -> Option<String> {
    Some("the DBI backend is not included in the fbsource Buck build".to_owned())
}
...
impl Backend {
    ...
    fn unavailable_reason(self) -> Option<String> {
        match self {
            ...
            #[cfg(fbcode_build)]
            Self::Kvm => Some("the KVM backend is not included in the fbsource Buck build".to_owned()),
        }
    }
}
...
async fn run_kvm(
    ...
    Err(anyhow!(
        "backend `kvm` is not included in the fbsource Buck build"
    ))
}
    ...
    fn optional_backends_report_accurate_availability() {
        ...
        match Backend::Kvm.ensure_available() {
            ...
            Err(kvm_error) => {
                ...
                assert!(
                    message.contains("/dev/kvm") || message.contains("not included"),
