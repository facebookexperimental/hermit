 ...
 *
 * This program issues an `open` syscall. Detcore rewrites `open` to `openat`
 * and *injects* it from Reverie's private trampoline page, which is exactly the
 * path that used to leave the trampoline's RIP in %rcx (verified: without the
 * fix,
...
int main(void) {
  ...
  if ((r11 & 0x2) == 0) {
    fprintf(
        stderr,
        "FAIL: r11 (0x%llx) does not look like RFLAGS\n",
  ...
  printf(
      "rcx_is_return_rip=1 r11_reserved_bit=%d open_ok=%d\n",
      (int)((r11 & 0x2) != 0),
      (int)(fd >= 0));
  return 0;
}
