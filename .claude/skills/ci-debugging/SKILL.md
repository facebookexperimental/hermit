...
For determinism or parity failures on any backend, use `--verify-strict` and
require a `--verify-json` verdict with `bitwise_parity: true` and nonzero
compared INFO-message counts. That compares exit status, stdout, and stderr
...
addresses. Default `--verify`, any output-only comparison, and more
aggressively normalized logs cannot establish canonical full-observation
parity; they may only help localize the first divergence.
